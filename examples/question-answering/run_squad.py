...
def set_seed(args):
    ...
    if args.use_pcl: pcl_bert.set_rnd_seed(args.seed)
    if args.n_gpu > 0:
...
def train(args, train_dataset, model, tokenizer):
    ...
    if args.use_pcl:
        optimizer = pcl_bert.AdamW(optimizer_grouped_parameters, lr=args.learning_rate, eps=args.adam_epsilon)
    else:
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
          ...
          with torch.autograd.profiler.profile(enabled=args.profile, use_cuda=(args.n_gpu > 0), record_shapes=record_shapes) as prof:
            ...
            if prof and args.use_pcl: pcl_bert.reset_debug_timers()
            ...
            if prof and args.use_pcl: pcl_bert.print_debug_timers()
